...
class CaseService:
    ...
    async def list_cases(
        ...
        total = await self.cases_collection.count_documents(filters)

        cases = await self.cases_collection.find(filters)\
            .skip((page - 1) * limit)\
            .limit(limit)\
            .sort("created_at", -1)\
            .to_list(limit)

        # Add Kenya API metadata to response
        kenya_metadata = None
    ...
    async def get_case_statistics(
        ...
        """Get case statistics with aggregation"""
        filters = {}
        if county:
            filters["county"] = county
    ...
    async def search_cases(self, query: str, limit: int = 20):
        """Search cases by description or case ID"""
        filters = {
            "$or": [
    ...
    async def _get_kenya_data_metadata(self):
        """Get metadata about Kenya API data"""
        try:
            latest = await self.kenya_data_collection.find_one(
                {},
                sort=[("fetched_at", -1)]
